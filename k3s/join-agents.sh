...
TOKEN=$(ssh ctrl-01 'sudo cat /var/lib/rancher/k3s/server/node-token')

join() {
  local host="$1" ip="$2"; shift 2
  echo "===== $host $ip $* ====="
  ssh "$host" "curl -sfL https://get.k3s.io | K3S_URL=${SERVER} K3S_TOKEN=${TOKEN} sh -s - agent --node-ip ${ip} $*"
}
