...
Default model is LiteLLM alias **`jarvis`** (router). Inspect prompts get LIVE telemetry via `scripts/owui-telemetry-filter.py` (OWUI global filter). Bottom-right **ROUTED** chip is the resolved LiteLLM child (LOCAL vs GROK), not the picker. Picker still has the raw ids as Tony's override.
